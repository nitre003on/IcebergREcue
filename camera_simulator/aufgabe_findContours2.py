...
try:
    ...
    while True:
        ...
        # ROI: Untere Hälfte des Bildes herausschneiden:
        unterer_teil = dieser_frame[int(PIXEL_HOEHE/2):,:,:]

        frame_als_hsv = cv2.cvtColor(unterer_teil, cv2.COLOR_BGR2HSV)
        #                                H  S  V
        ...
        for kontur in konturen:
            ...
            if cv2.contourArea(kontur) > 4000:
                ...
                # Rechteck um die Kontur zeichnen (um PIXEL_HOEHE/2 nach unten versetzt)
                cv2.rectangle(dieser_frame, (x, int(y + PIXEL_HOEHE/2)), (x + w, int(y + h + PIXEL_HOEHE/2)), ROT, 2)
