"""
Zeichnet ein Fadenkreuz in das Bild.
...
try:
    ...
    while True:
        ...
        start1 = (int((PIXEL_BREITE - 1) / 2), int(0))
        ende1 = (int((PIXEL_BREITE - 1) / 2), int(PIXEL_HOEHE - 1))
        cv2.line(dieser_frame, start1, ende1, GELB, 5)

        start2 = (int(0), int((PIXEL_HOEHE - 1) / 2))
        ende2 = (int(PIXEL_BREITE - 1)), int((PIXEL_HOEHE - 1) / 2)
        cv2.line(dieser_frame, start2, ende2, GELB, 5)
